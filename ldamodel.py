...
#TODO replace this to functions from _lda_model pyx (cython modul) it needs arrays
def dirichlet_expectation(alpha):
    """
    For a vector `theta~Dir(alpha)`, compute `E[log(theta)]`.

    """
    if (len(alpha.shape) == 1):#_dirichlet_expectation_1d
    ...
    def inference(self, chunk, collect_sstats=False):
        ...
        for d, doc in enumerate(chunk):
            ...
            for _ in xrange(self.iterations):
                ...
                # TODO replace this to call of mean_change function from _lda_model
